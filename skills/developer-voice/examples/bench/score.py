...
BANNED = [
    "great question", "good question", "absolutely right", "you're right",
    "great idea", "happy to help", "nice work", "well done", "kudos",
    "please note", "it is important to note", "at this time",
    "simply", " just ", "this is easy", "obviously", "in order to", "let's ",
    "tl;dr", "ymmv", "sanity check", "sanity-check", "short answer",
    "excited", "thrilled", "delighted", "awesome", "seamless", "blazing",
    "game-chang", "supercharge", "unlock", "empower", "leverage",
]
...
def main():
    ...
    totals = {"off": 0, "on": 0}
    print(f"{'prompt':22} {'arm':4} {'mean':>6} {'spread':>7} {'!':>3}  banned (prose | in-code only)")
    for p in prompts:
        for arm in ("off", "on"):
            rs = runs.get((p, arm), [])
            if not rs:
            ...
            for r in rs:
                ...
                for k, v in r["banned_full"].items():
                    bf[k] += v
            only_code = {k: v - bp.get(k, 0) for k, v in bf.items() if v - bp.get(k, 0) > 0}
            print(f"{p:22} {arm.upper():4} {st.mean(w):6.0f} {max(w) - min(w):7} "
                  f"{sum(r['bang'] for r in rs):3}  {dict(bp) or '-'} | {only_code or '-'}")
        print()

    delta = 100 * (totals["on"] - totals["off"]) / totals["off"]
    print(f"TOTAL prose words   OFF={totals['off']}  ON={totals['on']}  ({delta:+.0f}%)")
