...
def analyze(pos):
    ...
    assert (
        abs(total_p - 1.0) < 1e-3
    ), f'{pos["ticker"]}: probabilities sum to {total_p}, not 1.0'
    ...
    for s in pos["scenarios"]:
        ...
        scenarios.append(
            {**s, "effective_target": effective, "return_pct": round(ret * 100, 1)}
        )

    ev_return = sum(
        s["prob"] * (s["effective_target"] - price) / price for s in scenarios
    )
    return {
        "ticker": pos["ticker"],
        "price": price,
        ...
        json.dumps(
            {
                ...
                "portfolio": {
                    ...
                    "expected_return_pct": round(ev_change / total * 100, 2)
                    if total
                    else None,
                },
            },
    ...
    r = analyze(
        {
            ...
            "stop_loss": 90.0,
            "scenarios": [
                {"name": "bull", "prob": 0.3, "target": 120.0},
